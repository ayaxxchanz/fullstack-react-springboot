...
import { loginSuccess, logout } from "../store/auth-slice"; // replace context with redux
  ...
  useEffect(() => {
    if (actionData?.success) {
      if (actionData.profileData.emailUpdated) {
        ...
        dispatch(logout()); // replace context with redux
        toast.success(
        ...
        if (actionData.profileData) {
          ...
          // Update in context
          dispatch(loginSuccess({jwtToken: localStorage.getItem("jwtToken"), user: updatedUser})); // replace context with redux
        }
      }
    }
  }, [actionData]);
