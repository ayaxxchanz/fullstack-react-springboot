...
apiClient.interceptors.request.use(
    async (config) => {
        ...
        // lecture 340: commented to solve CSRF error
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    if (error.response && error.response.status === 401) {
      const jwtToken = localStorage.getItem("jwtToken"); // check if token exists to differentiate unauthorized due to no token or invalid/expired token
      if (jwtToken) {
