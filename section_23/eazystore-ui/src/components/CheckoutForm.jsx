...
import { useSelector, useDispatch } from "react-redux"; // replace context with redux
import { selectCartItems, selectTotalPrice, clearCart } from "../store/cart-slice"; // replace context with redux
...
export default function CheckoutForm() {
  ...
  const totalPrice = useSelector(selectTotalPrice); // replace context with redux
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // backend API to interact with Stripe API
      ...
      if (error) {
        ...
        try {
          ...
          sessionStorage.setItem("skipRedirectPath", "true"); // to prevent ProtectedRoute redirect loop if user logout and login again right after order success
          dispatch(clearCart());
  ...
  return (
    <div className="min-h-[852px] flex items-center justify-center font-primary dark:bg-darkbg">
      ...
      >
        <p className="mt-4 text-2xl font-normal text-primary dark:text-dark">
      ...
      <div
        className={
          isProcessing
            ? "hidden"
            : "visible bg-dark-2 dark:bg-light-2 shadow-md rounded-lg max-w-md w-full px-8 py-6"
        }
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <button
              ...
              className="w-full px-6 py-2 mt-6 text-dark bg-light dark:bg-dark hover:bg-gray-700 dark:text-light hover:dark:bg-gray-400 rounded-md transition duration-200"
            >
