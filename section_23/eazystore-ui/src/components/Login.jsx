...
import { loginSuccess } from "../store/auth-slice"; // replace context with redux
  ...
  const from = sessionStorage.getItem("redirectPath") || "/home";
   ...
   useEffect(() => {
    if (actionData?.success) {
      navigate("/home");
      dispatch(loginSuccess({jwtToken: actionData.jwtToken, user: actionData.user})); // replace context with redux
      sessionStorage.removeItem("redirectPath");
      // set a delay of 100 ms before navigating to let JWT token finish storing inside localStorage
      setTimeout(() => {
        navigate(from);
      }, 100);
